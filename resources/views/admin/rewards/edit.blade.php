...
@section('title', t('Edit Reward'))

@section('content')
<div class="container-fluid px-4">
    <h1 class="mt-4">{{ t('Edit Reward') }}</h1>
            ...
            <form method="POST" action="{{ route('admin.rewards.update', $reward) }}" enctype="multipart/form-data">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('points_cost') is-invalid @enderror" 
                                   id="points_cost" name="points_cost" value="{{ old('points_cost', $reward->points_cost) }}" 
                                   min="1" required>
                            @error('points_cost')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      id="description" name="description" rows="4" required>{{ old('description', $reward->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('stock_quantity') is-invalid @enderror" 
                                   id="stock_quantity" name="stock_quantity" value="{{ old('stock_quantity', $reward->stock_quantity) }}" 
                                   min="0" required>
                        ...
                        <div class="mb-3">
                            ...
                            @if($reward->image)
                                <div class="mt-2">
                                    <small class="text-muted">{{ t('Current image:') }}</small><br>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" value="1" 
                                       id="is_active" name="is_active" {{ old('is_active', $reward->is_active) ? 'checked' : '' }}>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check me-1"></i>{{ t('Update Reward') }}
                    </button>
                    <a href="{{ route('admin.rewards.index') }}" class="btn btn-secondary">
                        <i class="bi bi-arrow-left me-1"></i>{{ t('Back') }}
                    </a>
                    <a href="{{ route('admin.rewards.show', $reward) }}" class="btn btn-outline-info">
