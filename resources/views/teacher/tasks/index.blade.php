...
<div class="container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>{{ t('My Tasks') }}</h1>
        <a href="{{ route('teacher.tasks.today') }}" class="btn btn-primary">
            <i class="bi bi-calendar-day me-1"></i> {{ t('Today\'s Tasks') }}
        </a>
    </div>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($assignments as $assignment)
                                <tr class="{{ $assignment->assigned_date->isToday() ? 'table-info' : '' }}">
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('teacher.tasks.show', $assignment) }}" class="btn btn-sm btn-outline-info">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            @if(!$assignment->completion)
                                            ...
                                            @else
                                                <form action="{{ route('teacher.tasks.uncomplete', $assignment) }}" method="POST" class="d-inline">
                                                    @csrf
        ...
        <div class="modal fade" id="completeModal{{ $assignment->id }}" tabindex="-1" aria-labelledby="completeModalLabel{{ $assignment->id }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('teacher.tasks.complete', $assignment) }}" method="POST">
                        @csrf
