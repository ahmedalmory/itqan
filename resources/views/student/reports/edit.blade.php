...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3">{{ t('edit_daily_report') }}</h1>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('student.reports.update', $report) }}" method="POST" id="reportForm" class="needs-validation" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control" 
                           ...
                           value="{{ $report->report_date->format('Y-m-d') }}" readonly>
                    <div class="form-text">{{ t('The report date cannot be changed') }}</div>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('memorization_from_surah_id') is-invalid @enderror" 
                        ...
                        <option value="{{ $surah->id }}" {{ old('memorization_from_surah_id', $report->memorization_from_surah) == $surah->id ? 'selected' : '' }}>
                            {{ $surah->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" class="form-control @error('memorization_from_verse') is-invalid @enderror" 
                           ...
                           value="{{ old('memorization_from_verse', $report->memorization_from_verse) }}">
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('memorization_to_surah_id') is-invalid @enderror" 
                        ...
                        <option value="">{{ t('select_surah') }}</option>
                        @foreach($surahs as $surah)
                        <option value="{{ $surah->id }}" {{ old('memorization_to_surah_id', $report->memorization_to_surah) == $surah->id ? 'selected' : '' }}>
                            {{ $surah->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" class="form-control @error('memorization_to_verse') is-invalid @enderror" 
                           ...
                           value="{{ old('memorization_to_verse', $report->memorization_to_verse) }}">
            ...
            <div class="d-flex justify-content-end">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save me-1"></i> {{ t('update_report') }}
                </button>
            </div>
        </form>
    </div>
...
// Store parts values
let memorizationParts = parseFloat(document.getElementById('memorization_parts').value) || 0;
let revisionParts = parseFloat(document.getElementById('revision_parts').value) || 0;
